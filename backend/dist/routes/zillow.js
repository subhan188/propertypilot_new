 ...
 */
async function searchZillowListings(request, reply) {
 ...
 */
async function getZillowPropertyDetails(request, reply) {
    const userId = request.session.userId;
    const { zpidOrUrl } = request.body;
    try {
 ...
 */
async function importZillowProperty(request, reply) {
    const userId = request.session.userId;
    const { zpidOrUrl, type = 'flip', notes } = request.body;
    try {
        if (!zpidOrUrl || zpidOrUrl.trim().length === 0) {
            throw new ValidationError({
                zpidOrUrl: 'ZPID or Zillow URL is required',
 ...
 */
async function refreshZillowData(request, reply) {
    const userId = request.session.userId;
 ...
 */
async function getAPIUsageStats(request, reply) {
    try {
        const client = getHasDataClient();
        const usage = client.getUsageStats();
 ...
 */
export async function zillowRoutes(fastify) {
    // Search listings
