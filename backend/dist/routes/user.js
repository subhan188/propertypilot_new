...
async function getUserPreferences(request, reply) {
    const userId = request.session.userId;
    try {
...
async function updateUserPreferences(request, reply) {
    const userId = request.session.userId;
    const body = request.body;
        ...
        if (existing) {
            ...
            const created = await prisma.userPreferences.create({
                data: {
                    ...
                    propertyStatuses: propertyStatuses || ['lead', 'analyzing', 'offer', 'under_contract', 'owned'],
                },
            });
...
async function exportUserData(request, reply) {
    const userId = request.session.userId;
    try {
...
async function deleteUserAccount(request, reply) {
    const userId = request.session.userId;
    const body = request.body;
    const { password } = body;
    try {
...
export async function userRoutes(fastify) {
    ...
    fastify.put('/api/user/me', updateUserProfile);
    fastify.get('/api/user/preferences', getUserPreferences);
    fastify.put('/api/user/preferences', updateUserPreferences);
    fastify.get('/api/user/export-data', exportUserData);
    fastify.post('/api/user/delete-account', deleteUserAccount);
}
