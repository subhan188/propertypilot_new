...
async function getPortfolioReport(request, reply) {
...
async function getPropertyReport(request, reply) {
    const userId = request.session.userId;
...
async function exportPortfolioReport(request, reply) {
    const userId = request.session.userId;
...
async function exportPropertyReport(request, reply) {
    const userId = request.session.userId;
    const { propertyId } = request.params;
    const { format = 'json' } = request.query;
    try {
        ...
        if (format === 'csv') {
            const csv = reportService.convertToCSV(report);
            reply.header('Content-Type', 'text/csv');
            reply.header('Content-Disposition', `attachment; filename="property-${propertyId}-report.csv"`);
            return reply.send(csv);
        }
        else if (format === 'json') {
            const json = reportService.convertToJSON(report);
            reply.header('Content-Type', 'application/json');
            reply.header('Content-Disposition', `attachment; filename="property-${propertyId}-report.json"`);
            return reply.send(json);
        }
        else {
            return reply.code(400).send({
...
export async function reportRoutes(fastify) {
    ...
    fastify.get('/api/reports/property/:propertyId', getPropertyReport);
    fastify.get('/api/reports/portfolio/export', exportPortfolioReport);
    fastify.get('/api/reports/property/:propertyId/export', exportPropertyReport);
}
