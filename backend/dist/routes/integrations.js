...
async function getIntegrations(request, reply) {
...
async function connectService(request, reply) {
    const userId = request.session.userId;
...
async function disconnectService(request, reply) {
    const userId = request.session.userId;
    const { service } = request.params;
    try {
    ...
    try {
        ...
        switch (service.toLowerCase()) {
            ...
            case 'mls':
                ...
                adapter = new MLSAdapter((config?.region || 'default'), integration.apiKey || undefined);
                break;
            default:
                throw new ValidationError({
                    service: 'Unsupported service',
...
export async function integrationRoutes(fastify) {
    ...
    fastify.post('/api/integrations/:service/connect', connectService);
    fastify.delete('/api/integrations/:service/disconnect', disconnectService);
    fastify.post('/api/integrations/:service/test', testConnection);
}
