...
import { calculateNOI, calculateCapRate, calculateCashOnCash, calculateROI, calculateFlipProfit, calculateIRR, calculateNPV, mortgageAmortizationSchedule, } from '@/calculations/financials';
import { NotFoundError, ValidationError } from '@/utils/errors';
async function analyzeScenarioHandler(request, reply) {
...
async function mortgageScheduleHandler(request, reply) {
...
async function compareScenariosHandler(request, reply) {
    const body = request.body;
...
export async function analysisRoutes(fastify) {
    fastify.post('/scenarios/:scenarioId/analyze', analyzeScenarioHandler);
    fastify.post('/analysis/mortgage-schedule', mortgageScheduleHandler);
    fastify.post('/analysis/compare-scenarios', compareScenariosHandler);
}
/**
 * Calculate financial metrics for a scenario
 */
function calculateScenarioMetrics(scenario, data) {
    ...
    if (strategy === 'flip') {
        // Flip calculation
        totalProfit = calculateFlipProfit(scenario.purchasePrice, scenario.rehabCost, scenario.closingCosts, scenario.salePrice || 0, scenario.sellingCosts || 0, scenario.holdingCosts || 0);
        roi = calculateROI(totalProfit, scenario.purchasePrice +
            scenario.rehabCost +
        ...
        try {
            irr = calculateIRR(cashFlows, scenario.purchasePrice +
                scenario.rehabCost +
                scenario.closingCosts +
                (scenario.holdingCosts || 0));
        }
        ...
        // Rental calculation
        const expenses = (scenario.monthlyExpenses || 0) + (scenario.maintenanceCost || 0);
        monthlyNOI = calculateNOI(scenario.monthlyRent || 0, scenario.occupancyRate || 100, expenses) / 12;
        ...
        const monthlyPayment = (scenario.interestRate || 7) > 0
            ? monthlyNOI * 0.8 // Conservative: 80% available
            : monthlyNOI;
        const cashFlows = Array(12 * (scenario.holdTimeMonths || 5)).fill(monthlyPayment);
        try {
        ...
        const expenses = (scenario.monthlyExpenses || 0) + (scenario.cleaningCost || 0);
