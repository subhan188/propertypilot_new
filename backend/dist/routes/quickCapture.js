...
async function quickCaptureHandler(request, reply) {
...
async function quickCaptureBatchHandler(request, reply) {
    const userId = request.session.userId;
    ...
    try {
        ...
        const validated = propertyList.map((p) => quickCaptureSchema.parse(p));
        // Create all properties
        const created = await Promise.all(validated.map((p) => propertyService.createProperty(userId, {
            address: p.address,
            city: p.city,
            state: p.state,
            zipCode: p.zipCode,
...
export async function quickCaptureRoutes(fastify) {
    fastify.post('/properties/quick-capture', quickCaptureHandler);
    fastify.post('/properties/quick-capture/batch', quickCaptureBatchHandler);
}
