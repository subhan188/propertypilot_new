 ...
 */
export function calculateNOI(monthlyRent, occupancyRate, monthlyExpenses = 0) {
 ...
 */
export function calculateCashOnCash(monthlyNOI, downPayment) {
    if (downPayment <= 0) {
 ...
 */
export function calculateROI(totalProfit, totalInvested) {
    if (totalInvested <= 0) {
 ...
 */
export function calculateFlipProfit(purchasePrice, rehabCost, closingCosts, salePrice, sellingCosts, holdingCosts = 0) {
    const totalInvested = purchasePrice + rehabCost + closingCosts + holdingCosts;
 ...
 */
export function mortgageAmortizationSchedule(principal, annualRate, months) {
 ...
 */
export function estimateARV(comparables, propertySquareFeet) {
    ...
    const medianPricePerSqft = pricePerSqftList.length % 2 === 0
 ...
 */
export function calculateBreakEvenMonths(monthlyNOI, initialInvestment) {
    if (monthlyNOI === 0) {
 ...
 */
export function calculateNPV(cashFlows, initialInvestment, discountRate) {
    if (!cashFlows || cashFlows.length === 0) {
        throw new Error('At least one cash flow is required');
    }
 ...
 */
export function sensitivityAnalysis(baseCase, variationPercent = 10) {
    const baseProfit = calculateFlipProfit(baseCase.purchasePrice, baseCase.rehabCost, baseCase.closingCosts, baseCase.salePrice, baseCase.sellingCosts);
    const variation = variationPercent / 100;
    const scenarios = [
        {
            variable: 'salePrice',
            downPercent: calculateFlipProfit(baseCase.purchasePrice, baseCase.rehabCost, baseCase.closingCosts, baseCase.salePrice * (1 - variation), baseCase.sellingCosts),
            upPercent: calculateFlipProfit(baseCase.purchasePrice, baseCase.rehabCost, baseCase.closingCosts, baseCase.salePrice * (1 + variation), baseCase.sellingCosts),
        },
        {
            variable: 'rehabCost',
            downPercent: calculateFlipProfit(baseCase.purchasePrice, baseCase.rehabCost * (1 - variation), baseCase.closingCosts, baseCase.salePrice, baseCase.sellingCosts),
            upPercent: calculateFlipProfit(baseCase.purchasePrice, baseCase.rehabCost * (1 + variation), baseCase.closingCosts, baseCase.salePrice, baseCase.sellingCosts),
        },
        {
            variable: 'purchasePrice',
            downPercent: calculateFlipProfit(baseCase.purchasePrice * (1 - variation), baseCase.rehabCost, baseCase.closingCosts, baseCase.salePrice, baseCase.sellingCosts),
            upPercent: calculateFlipProfit(baseCase.purchasePrice * (1 + variation), baseCase.rehabCost, baseCase.closingCosts, baseCase.salePrice, baseCase.sellingCosts),
        },
    ];
