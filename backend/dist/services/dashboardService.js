...
export class DashboardService {
    async getKPI(userId) {
        ...
        const monthlyCashflow = scenarios
            .filter((s) => s.exitStrategy !== 'flip' && s.propertyId)
            .reduce((sum, s) => sum + (s.monthlyNOI || 0), 0);
        // Calculate equity (assuming 75% LTV on average)
        const availableEquity = portfolioValue * 0.25; // Simplified: 25% equity
        // Count properties by status
        const ownedProperties = properties.filter((p) => p.status === 'owned').length;
        const activeDeals = properties.filter((p) => ['analyzing', 'offer', 'under_contract'].includes(p.status)).length;
        const kpi = {
    ...
    async getPortfolioTrend(userId, days = 90) {
        const properties = await prisma.property.findMany({
            where: { userId },
        });
        // Generate mock trend data based on portfolio value
        const currentValue = properties.reduce((sum, p) => sum + p.currentValue, 0);
    ...
    async getPropertyTypes(userId) {
        const properties = await prisma.property.findMany({
            where: { userId },
        });
        const breakdown = {
            rent: {
                count: properties.filter((p) => p.type === 'rent').length,
                value: properties
                    .filter((p) => p.type === 'rent')
                    .reduce((sum, p) => sum + p.currentValue, 0),
            },
            airbnb: {
                count: properties.filter((p) => p.type === 'airbnb').length,
                value: properties
                    .filter((p) => p.type === 'airbnb')
                    .reduce((sum, p) => sum + p.currentValue, 0),
            },
            flip: {
                count: properties.filter((p) => p.type === 'flip').length,
                value: properties
                    .filter((p) => p.type === 'flip')
                    .reduce((sum, p) => sum + p.currentValue, 0),
            },
        };
        return breakdown;
