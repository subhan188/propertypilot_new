...
export class FileService {
     ...
     */
    async uploadFile(userId, propertyId, renovationId, filename, buffer, mimeType) {
        ...
        const resourceId = (renovationId || propertyId || userId);
        const s3Key = S3Service.generateKey(resource, resourceId, filename);
        // Upload to S3
        const { key, signedUrl, expiresAt } = await s3Service.uploadFile(s3Key, buffer, mimeType);
        // Save to database
     ...
     */
    async getPropertyFiles(userId, propertyId) {
        // Verify property belongs to user
        const property = await prisma.property.findFirst({
            where: { id: propertyId, userId },
        });
     ...
     * @returns Array of FileUpload objects
     */
    async getRenovationFiles(userId, renovationId) {
     ...
     */
    async refreshSignedUrl(userId, fileId) {
        const file = await prisma.fileUpload.findUnique({
            where: { id: fileId },
            include: {
                property: { select: { userId: true } },
